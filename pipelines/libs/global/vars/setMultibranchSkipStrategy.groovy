...
def call(String regex = '\\[(ci skip|skip ci)\\]', def files = null, boolean dryrun = false) {
    ...
    println("setMultibranchSkipStrategy for ${owner.fullName}:")
    println(" commit-skip regex:    ${regex == null ? '(unchanged)' : regex}")
    println(" changed-file regions: ${regions == null ? '(unchanged)' : (regions.trim() ?: '(cleared)')}")
    println(" result:               ${changed ? (dryrun ? 'would change (dry run)' : 'saved') : 'already up to date'}")
}
