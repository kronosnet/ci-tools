...
def call()
{
    ...
    providers['az'] = ['maxjobs': 4, 'testlevel': 'all', 'rhelvers': ['7', '8', '9'],
		       'has_watchdog': true, 'has_storage': true, 'weekly': false,
		       'defaultiscsi': '',
		       'defaultuseiscsi': 'no',
		       'defaultblocksize': '',
		       'authopts': '--region eastus',
		       'rhel7': ['createopts': '--image $(az vm image list --all --publisher RedHat  --output table | grep "RedHat:RHEL:7_" | tail -n 1 | awk \'{print $NF}\') --flavor Standard_D2s_v3 --flavor-workstation Standard_D2s_v3',
				 'deployopts':  '',
				 'testopts': '',
				 'setup_fn': {}],
		       'rhel8': ['createopts': '--image $(az vm image list --all --publisher RedHat  --output table | grep "RedHat:RHEL:8_" | tail -n 1 | awk \'{print $NF}\') --flavor Standard_D2s_v3 --flavor-workstation Standard_D2s_v3',
				 'deployopts':  '',
				 'testopts': '',
				 'setup_fn': {}],
		       'rhel9': ['createopts': '--image $(az vm image list --all --publisher RedHat  --output table | grep "RedHat:RHEL:9_" | tail -n 1 | awk \'{print $NF}\') --flavor Standard_D2s_v3 --flavor-workstation Standard_D2s_v3',
				 'deployopts':  '',
				 'testopts': '',
				 'setup_fn': {}]]
