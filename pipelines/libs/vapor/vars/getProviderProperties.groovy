...
def call()
{
    ...
    providers['aws'] = ['maxjobs': 4, 'testlevel': 'smoke', 'vers': ['rhel8', 'rhel9', 'rhel10'],
			...
			'rhel8': ['createopts': '--image $(vapor get-images aws --region us-east-1 | grep "RHEL-8.10.0_HVM-" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				  ...
				  'testopts': '',
				  'setup_fn': {}],
			'rhel9': ['createopts': '--image $(vapor get-images aws --region us-east-1 | grep "RHEL-9.7.0_HVM_GA" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}],
			'rhel10': ['createopts': '--image $(vapor get-images aws --region us-east-1 | grep "RHEL-10.1.0_HVM_GA" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				   'deployopts':  '',
				   'testopts': '',
				   'setup_fn': {}]]
