...
def call()
{
    ...
    providers['libvirt'] = ['maxjobs_smoke': 4, 'maxjobs_all': 4, 'testlevel': 'smoke', 'vers': ['rhel8', 'rhel9', 'rhel10', 'centos10'],
			    ...
			    'rhel8': ['createopts': "--image rhel-8.10.0.x86_64.qcow2 --flavor-workstation rhelha-vapor-workstation-medium --flavor rhelha-vapor-node-medium",
				      ...
				      'testopts': '',
				      'setup_fn': {}],
			    'rhel9': ['createopts': "--image rhel-9.8.0.x86_64.qcow2 --flavor-workstation rhelha-vapor-workstation-medium --flavor rhelha-vapor-node-medium",
				      'deployopts':  '',
				      'testopts': '',
				      'setup_fn': {}],
			    'rhel10': ['createopts': "--image rhel-10.2.0.x86_64.qcow2 --flavor-workstation rhelha-vapor-workstation-medium --flavor rhelha-vapor-node-medium",
				       'deployopts':  '',
				       'testopts': '',
				       'setup_fn': {}],
    ...
    providers['osp'] = ['maxjobs_smoke': 4, 'maxjobs_all': 8, 'testlevel': 'all', 'vers': ['rhel8', 'rhel9'],
			...
			'rhel8': ['createopts': '--image rhel-8.10.0.x86_64',
				  ...
				  'testopts': '',
				  'setup_fn': {}],
			'rhel9': ['createopts': '--image rhel-9.8.0.x86_64',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}]]
    ...
    providers['aws'] = ['maxjobs_smoke': 4, 'maxjobs_all': 4, 'testlevel': 'smoke', 'vers': ['rhel8', 'rhel9', 'rhel10'],
			...
			'rhel8': ['createopts': '--image $(vapor get-images aws --region us-east-2 | grep "RHEL-8.10.0_HVM-" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				  ...
				  'testopts': '',
				  'setup_fn': {}],
			'rhel9': ['createopts': '--image $(vapor get-images aws --region us-east-2 | grep "RHEL-9.8.0_HVM_GA" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}],
			'rhel10': ['createopts': '--image $(vapor get-images aws --region us-east-2 | grep "RHEL-10.2.0_HVM_GA" | grep x86_64 | sed -e \'s#",##g\' -e \'s#.*"##g\' | sort -u | tail -n 1)',
				   'deployopts':  '',
				   'testopts': '',
				   'setup_fn': {}]]
    ...
    providers['gcp'] = ['maxjobs_smoke': 4, 'maxjobs_all': 4, 'testlevel': 'smoke', 'vers': ['rhel8', 'rhel9', 'rhel10'],
			...
			'rhel8': ['createopts': '--image $(vapor get-images gcp 2>/dev/null | jq --raw-output \'.[].name\' | grep "^rhel-8-v" | sort -V | tail -n 1)',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}],
			'rhel9': ['createopts': '--image $(vapor get-images gcp 2>/dev/null | jq --raw-output \'.[].name\' | grep "^rhel-9-v" | sort -V | tail -n 1)',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}],
			'rhel10': ['createopts': '--image $(vapor get-images gcp 2>/dev/null | jq --raw-output \'.[].name\' | grep "^rhel-10-v" | sort -V | tail -n 1)',
				  'deployopts':  '',
				  'testopts': '',
				  'setup_fn': {}]]
    ...
    providers['aliyun'] = ['maxjobs_smoke': 4, 'maxjobs_all': 4, 'testlevel': 'smoke', 'vers': ['rhel8', 'rhel9', 'rhel10'],
			   ...
			   'rhel9': ['createopts': '--image rhel-9.8.0.x86_64',
				     ...
				     'testopts': '',
				     'setup_fn': {}],
			   'rhel10': ['createopts': '--image rhel-10.2.x86_64',
				      'deployopts':  '',
				      'testopts': '',
				      'setup_fn': {}]]
    ...
    providers['ocpv'] = ['maxjobs_smoke': 4, 'maxjobs_all': 0, 'testlevel': 'all', 'vers': ['rhel8', 'rhel9', 'rhel10', 'centos10'],
			 ...
			 'rhel8': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-8.10.0',
				   ...
				   'testopts': '',
				   'setup_fn': {}],
			 'rhel9': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-9.8.0',
				   'deployopts':  '',
				   'testopts': '',
				   'setup_fn': {}],
			 'rhel10': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-10.2.0',
				    'deployopts':  '',
				    'testopts': '',
				    'setup_fn': {}],
