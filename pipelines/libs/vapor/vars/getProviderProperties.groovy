...
def call()
{
    ...
    providers['libvirt'] = ['maxjobs': 4, 'testlevel': 'smoke', 'vers': ['rhel8'],
    ...
    providers['osp'] = ['maxjobs': 4, 'testlevel': 'all', 'vers': ['rhel9'],
			'has_watchdog': true, 'has_storage': true, 'has_network': true, 'weekly': true,
			'api_rate_limit': true,
			'defaultiscsi': '200',
			'defaultuseiscsi': 'yes',
    ...
    providers['ocpv'] = ['maxjobs': 4, 'testlevel': 'all', 'vers': ['rhel8', 'rhel9', 'rhel10', 'centos10'],
			 ...
			 'rhel8': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-8.10.0',
				   ...
				   'testopts': '',
				   'setup_fn': {}],
			 'rhel9': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-9.6.0',
				   'deployopts':  '',
				   'testopts': '',
				   'setup_fn': {}],
			 'rhel10': ['createopts': '--flavor-workstation u1.2xlarge --flavor u1.2xlarge --block-emulation-type scsi --image rhel-10.0.0',
				    'deployopts':  '',
				    'testopts': '',
				    'setup_fn': {}],
